...
- `icon`: Standard keys: `"play"`, `"pause"`, `"play-pause"`, `"stop"`, `"record"`, `"fast-forward"`, `"rewind"`, `"skip-forward"`, `"skip-backward"`, `"arrow-left"`, `"arrow-right"`, `"eject"`. Waveform keys: `"sine-wave"`, `"square-wave"`, `"triangle-wave"`, `"sawtooth-wave"`, `"pulse-wave"`, `"sample-hold"`, `"sample-glide"`, `"noise"`. DJ keys: `"cue"`, `"vinyl-mode"`, `"loop-in"`, `"loop-out"`, `"beat-sync"`, `"slip"`, `"quantize"`. Only set if `labelDisplay` is `"icon-only"` or `"right"` (for LED indicators with icons).
- **Indicator splitting (UNCONDITIONAL when count > 1):** When the diagram parser reports `type: "led-group"` with `count > 1`, you MUST emit one control per LED — do NOT merge into a single control regardless of how the manual phrases it. Geometric evidence (parser count) overrides textual phrasing every time. Use the parser's `waveforms` / `voiceNumbers` / `octaveLabels` array (whichever applies) to name each control. ID format: `{section}-{name}` for named LEDs (e.g., `lfo1-sine`, `lfo1-square`, `lfo1-ramp-up`, `lfo1-ramp-down`, `lfo1-sample-hold`, `lfo1-sample-glide`); `{section}-{N}` for numbered LEDs (e.g., `voice-1` … `voice-12`); `{section}-{label}` for octave-style labeled LEDs (e.g., `octave-minus-2`, `octave-zero`, `octave-plus-2`). Set `type: "led"`, `icon` to the matching waveform key when applicable, `labelDisplay: "right"`. The post-gatekeeper validator will fail the run if any `led-group` cluster ends up with fewer split controls than the parser counted.
...
- `ledStyle`: One of `"dot"`, `"face"` (alias `"integrated"`), `"label-backlit"`, `"edge-glow"`, or `null`. Decision tree based on the hardware photo:
  - **`null`** + `hasLed: false` — no LED visible on or around the button (SHIFT, USB STOP).
  - **`"dot"`** — a separate small indicator sits NEAR the button; the dot lights up but the button face stays dark. Synth LFO/voice/octave LEDs, side-panel indicators.
  - **`"face"`** — whole button face glows uniformly when active. Most common DJ pattern: PLAY, CUE, HOT CUE, performance pads, mode toggles (SLIP, QUANTIZE, SYNC). Emit `"face"` for new manifests; `"integrated"` is a permanent back-compat alias that renders identically (do NOT migrate existing data).
  - **`"label-backlit"`** — face stays dark; only the label/icon glows through cutouts. Pioneer DJM mixers, NI Maschine transport.
  - **`"edge-glow"`** — face stays dark; only the border/ring glows. On `shape: "circle"` this auto-renders as a ring (the `border-radius: 50%` turns a CSS border into a ring — no separate "ring" style).
  
  If uncertain after studying the photo, default to `"face"` and note the uncertainty in your checkpoint conflicts list. Admin can adjust via the editor's 5-pill Properties Panel.
...
- `keyboard`: `{ keys, startNote, type, panelHeightPercent }` or `null`
  ...
  - **MANDATORY when parser detected keys:** if the Diagram Parser's `spatial-blueprint.json` contains a `keyboard.keys` value > 0, you MUST emit a populated `keyboard` field — regardless of any parser notes like "below the panel," "outside the coordinate system," or "not included in panel area." Those notes describe the parser's chosen coordinate framing (control-strip only), NOT a directive to omit the keyboard from your manifest. Setting `keyboard: null` when the parser detected keys is a HARD ERROR — the downstream editor renders only the control strip, no piano, and tutorials cannot recover from it. If unsure about `panelHeightPercent`, estimate from the hardware photo's ratio of control-area height to total instrument height. (Fantom-07 hit this 2026-05-28: parser said "76 keys, below control panel"; gatekeeper emitted `keyboard: null` + `panelHeight: 178`; editor showed no keyboard.)
