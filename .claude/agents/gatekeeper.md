...
- **Indicator splitting**: If the manual describes multiple indicators collectively ("these LEDs indicate", "waveform LEDs"), create one control per individual indicator using the diagram parser's count. ID format: `{section}-{name}` (e.g., `lfo1-sine`, `lfo1-square`). Set `type: "led"`, `icon` to the matching waveform key, `labelDisplay: "right"`.
