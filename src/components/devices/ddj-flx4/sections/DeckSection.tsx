...
export default function DeckSection({ deck, panelState, highlightedControls, onButtonClick }: DeckSectionProps) {
    ...
    <div className="flex items-center justify-center gap-4 flex-1">
      {deck === 1 ? (
        <>
          ...
          <JogWheel id={`${d}-jog`} highlighted={isHl(`${d}-jog`)} size={550} onButtonClick={onButtonClick} />
  ...
  // Mode buttons: [SHIFT] [HOT CUE] [PAD FX1] [BEAT JUMP] [SAMPLER]
  const modeButtons = (
  ...
  // Row 4: Transport + Pads — CUE/PLAY on inner side, pads on outer
  ...
  return (
    ...
    >
      ...
      {/* Row 3: SHIFT + Mode buttons */}
      {modeButtons}

      {/* Row 4: Transport + Pads */}
      {transportAndPads}
    </div>
  );
}
