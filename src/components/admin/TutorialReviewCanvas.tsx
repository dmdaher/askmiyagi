...
import SidebarTabs, { type TabDef, type TabSeverity } from './SidebarTabs';
...
function TutorialReviewCanvasInner({ data, onRefreshData }: TutorialReviewCanvasProps) {
  ...
  // Sidebar tab state. Default = 'tutorials'. Tab content lives in the same
  // sidebar column; only the active tab's section renders to keep DOM lean.
  const [sidebarTab, setSidebarTab] = useState<'tutorials' | 'qa' | 'coverage' | 'notes'>('tutorials');
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-[#0a0a14] text-white" data-testid="tutorial-review-canvas">
      {/* ── Top bar ──────────────────────────────────────────────────── */}
      ...
      >
        ...
        <div className="flex flex-col overflow-y-auto min-h-0 border-r border-white/10 bg-[#0c0c18]" data-testid="canvas-sidebar">
          ...
          {/* ── Tab bar: switches sidebar between Tutorials / QA / Coverage / Notes */}
          <SidebarTabs
            activeId={sidebarTab}
            onChange={(id) => setSidebarTab(id as 'tutorials' | 'qa' | 'coverage' | 'notes')}
            tabs={[
              { id: 'tutorials', label: 'Tutorials', count: tutorials.length, severity: 'info' as TabSeverity },
              {
                id: 'qa',
                label: 'QA',
                count: qaReport ? qaReport.results.filter((r) => r.severity === 'fail').length + qaReport.results.filter((r) => r.severity === 'warn').length : null,
                severity: qaReport?.hasFailures ? 'error' as TabSeverity : qaHasIssues ? 'warn' as TabSeverity : 'success' as TabSeverity,
              },
              { id: 'coverage', label: 'Coverage', count: null },
              { id: 'notes', label: 'Notes', count: reviewerNoteEntries.length || null, severity: 'info' as TabSeverity },
            ]}
          />

          {sidebarTab === 'tutorials' && (
            <div className="flex-shrink-0" data-testid="tab-content-tutorials">
              <TutorialListPanel
                tutorials={tutorials}
                summary={summary}
          ...
          {sidebarTab === 'coverage' && (
            <div className="flex-shrink-0" data-testid="tab-content-coverage">
              <CoverageTab deviceId={deviceId} />
            </div>
          )}

          {/* QA Findings (auto-generated by pipeline at tutorial-review pause) */}
          {sidebarTab === 'qa' && qaReport && (
          ...
          {/* PR-K: Layer 5 Conceptual Coherence — lives in QA tab */}
          {sidebarTab === 'qa' && (
            <Layer5Panel
              deviceId={deviceId}
          ...
          {/* Reviewer prose accordion (god-mode) — lives in Notes tab */}
          {sidebarTab === 'notes' && reviewerNoteEntries.length > 0 && (
