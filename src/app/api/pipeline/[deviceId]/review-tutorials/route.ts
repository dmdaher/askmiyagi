...
export async function GET(
  ...
  try {
    ...
    // Canvas QA report — auto-generated by the pipeline at tutorial-review
    // pause; surfaced in the admin canvas as the QA Findings panel.
    let qaReport: unknown = null;
    if (fs.existsSync(qaReportPath)) {
      try { qaReport = JSON.parse(fs.readFileSync(qaReportPath, 'utf-8')); }
      catch (err) { console.warn(`[review-tutorials] failed to parse qa-report.json: ${err}`); }
    }
